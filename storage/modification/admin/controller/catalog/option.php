...
//karapuz (Advanced Filter) 
		if (class_exists('\KaGlobal') && \KaGlobal::isKaInstalled('ka_adv_filter')) {
			...
			if (isset($this->request->post['linked_fg_id'])) {
				$data['linked_fg_id'] = $this->request->post['linked_fg_id'];
			} elseif (!empty($option_info)) {
				$data['linked_fg_id'] = $option_info['linked_fg_id'];
			} else {
				$data['linked_fg_id'] = '';
			}
...
//karapuz (Advanced Filter) 
		$components = array();
		if (class_exists('\KaGlobal') && \KaGlobal::isKaInstalled('ka_adv_filter')) {
			$model_ka_adv_filter_option = $this->load->kamodel('extension/ka_extensions/ka_adv_filter/option');
...
//karapuz (Advanced Filter) 
			if (class_exists('\KaGlobal') && \KaGlobal::isKaInstalled('ka_adv_filter')) {
				$components = $model_ka_adv_filter_option->getComponents($option_value['option_value_id']);
			}
///karapuz (Advanced Filter)
			$data['option_values'][] = array(
//karapuz (Advanced Filter) 
...
//karapuz (Advanced Filter) 
	public function autocomplete_option_values() {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			$results = $model_ka_adv_filter_option->findOptionValues($filter_data);
