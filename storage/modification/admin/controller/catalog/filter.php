...
//karapuz (Advanced Filter) 
		if (class_exists('\KaGlobal') && \KaGlobal::isKaInstalled('ka_adv_filter')) {
			...
			$data['is_adv_filter_installed'] = true;
		
			$model_adv_filter_option = $this->load->kamodel('extension/ka_extensions/ka_adv_filter/option');
			
			if (!empty($filter_group_info)) {
				$data['source_option'] = $model_adv_filter_option->getSourceOption($filter_group_info['filter_group_id']);
				if (!empty($data['source_option'])) {
					$data['source_option']['url'] = $this->url->link('catalog/option/edit', 'user_token=' . $this->session->data['user_token'] . '&option_id=' . $data['source_option']['option_id'], true);
				}
			}
			...
			// show the filter as images
			if (isset($this->request->post['show_as_image'])) {
				$data['show_as_image'] = $this->request->post['show_as_image'];
			} elseif (!empty($filter_group_info)) {
				$data['show_as_image'] = $filter_group_info['show_as_image'];
			} else {
				$data['show_as_image'] = '';
			}
		}
