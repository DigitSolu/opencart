...
//karapuz (install.xml) 
		...
		$ka_info = $this->model_setting_modification->getModificationByCode('ka_extensions');
		if (empty($ka_info)) {
			$data['ka_extensions_error']    = 'not_available';
		} elseif (!method_exists($this->load, "kamodel")) {
			$data['ka_extensions_error']    = 'not_in_cache';
			$data['modifications_page_url'] = $this->url->link('marketplace/modification', 'user_token=' . $this->session->data['user_token'], true);
		}
///karapuz (install.xml) 
