...
class ModelCatalogReview extends Model {
	...
	public function getTotalReviews($data = array()) {
		...
		return $query->row['total'];
			...
			public function QuickStatus($review_id, $data) {
			...
			public function MultiQuickStatus($review_id, $data) {
				$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "review WHERE review_id = '" . (int)$review_id . "'");
				if ($query->row['status'] == $data['status']){
					$this->db->query("UPDATE " . DB_PREFIX . "review SET status = '".$data['status']."' WHERE review_id = '" . (int)$review_id . "'");
				} else{
					$this->db->query("UPDATE " . DB_PREFIX . "review SET status = '".$data['status']."' WHERE review_id = '" . (int)$review_id . "'");
				}
				return $query->row;
