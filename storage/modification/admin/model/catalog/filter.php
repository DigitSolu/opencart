...
//karapuz (Advanced Filter) 
		if (class_exists('\KaGlobal') && \KaGlobal::isKaInstalled('ka_adv_filter')) {
			$show_as_image = isset($data['show_as_image']) ? 1:0;
			$this->db->query("UPDATE " . DB_PREFIX . "filter_group 
				SET show_as_image = $show_as_image
...
//karapuz (Advanced Filter) 
				if (isset($filter['image'])) {
					$this->db->query("UPDATE `" . DB_PREFIX . "filter` SET image = '" . $this->db->escape($filter['image']) . "'
