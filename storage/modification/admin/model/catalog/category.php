...
class ModelCatalogCategory extends Model {
	...
	public function getTotalCategories() {
		...
		return $query->row['total'];
			...
			public function QuickStatus($category_id) {
			...
			public function MultiQuickStatus($category_id, $data) {
				$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "category WHERE category_id = '" . (int)$category_id . "'");
				if ($query->row['status'] == $data['status']){
					$this->db->query("UPDATE " . DB_PREFIX . "category SET status = '".$data['status']."' WHERE category_id = '" . (int)$category_id . "'");
				} else{
					$this->db->query("UPDATE " . DB_PREFIX . "category SET status = '".$data['status']."' WHERE category_id = '" . (int)$category_id . "'");
				}
				return $query->row;
