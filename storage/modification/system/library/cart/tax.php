...
//karapuz (Advanced Filter) 
	...
	protected function updateGeoZone($country_id, $zone_id) {
	
		// use the default geo zone (id=0) for a store where prices are displayed as is (i.e. without taxes)
		if ($this->config->get('config_tax') == 0) {
			$this->geo_zone_id = 0;
			return;
		}
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone zgz
			INNER JOIN " . DB_PREFIX . "tax_rate tr ON tr.geo_zone_id = zgz.geo_zone_id
			WHERE zgz.country_id = '" . (int)$country_id . "' AND (zgz.zone_id = '" . (int)$zone_id . "')
			LIMIT 1
		");
		if (empty($query->rows)) {
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone zgz
				INNER JOIN " . DB_PREFIX . "tax_rate tr ON tr.geo_zone_id = zgz.geo_zone_id
				WHERE zgz.country_id = '" . (int)$country_id . "' AND (zgz.zone_id = 0)
				LIMIT 1
			");
			if (empty($query->rows)) {
				$this->geo_zone_id = 0;
		...
		$this->geo_zone_id = $query->row['geo_zone_id'];		
	}
///karapuz (Advanced Filter)
	public function setShippingAddress($country_id, $zone_id) {
//karapuz (Advanced Filter) 
		$this->updateGeoZone($country_id, $zone_id);
///karapuz (Advanced Filter)
