<?php
//==============================================================================
// Category & Product-Based Fee/Discount v2023-5-01
// 
...
//namespace Opencart\Catalog\Model\Extension\CategoryProductBasedFee\Total;
//class CategoryProductBasedFee extends \Opencart\System\Engine\Model {
...
class ModelExtensionTotalCategoryProductBasedFee extends Model {
	...
	public function getTotal($total_input) {
		...
		$customer_group_id = ($this->customer->isLogged()) ? (int)$this->customer->getGroupId() : 0;
		$distance = 0;
		$distance_origin = '';
					...
					foreach ($product_group_ids as $product_group_id) {
						...
						foreach ($members_array as $type => $members) {
							...
							if (($comparison == 'all' || $comparison == 'onlyall') && array_diff($members, ${$type.'s_array'})) {
								$this->logMessage('Disabled for violating product group rule "' . $product_group_rule_text . '", due to missing ' . $type . '_id(s) "' . implode(', ', array_diff($members, ${$type.'s_array'})) . '"');
								continue 4;
							}
							...
							foreach ($cart_products as $product) {
								...
								if ((($comparison == 'onlyany' || $comparison == 'onlyall') && array_diff(${$type.'s'}[$product['key']], $members)) ||
									...
									$this->logMessage('Disabled for violating product group rule "' . $product_group_rule_text . '"');
									continue 5;
		...
		foreach ($settings['combination'] as $combination) {
			...
			if (!empty($combination['groups_required'])) {
				foreach (explode(',', strtolower($combination['groups_required'])) as $group) {
					if (!in_array(trim($group), array_keys($charges))) {
						$this->logMessage('Disabled because group ' . $group . ' was required and is not present');
						continue 2;
					}
				}
			}
			...
			if (empty($combined_title)) continue;
