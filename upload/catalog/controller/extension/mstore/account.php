...
class ControllerExtensionMstoreAccount extends Controller {
	...
	/**
	 * @api {get} /index.php?route=extension/mstore/account/addresses Get user's addresses
	 * @apiVersion 0.1.0
	 * @apiName Get user's addresses
	 * @apiGroup Account
	 *
	 * @apiSuccess {Array} data  List Address.
	 * @apiSuccess {Array} error  List error messages.
	 * 
	 */
	public function addresses() {
		...
		if (!$this->customer->isLogged()) {
			$this->response->addHeader('HTTP/1.0 401 Unauthorized');
			$this->response->setOutput(json_encode(["success"=>0, "error"=>["Please login to get user's addresses"], "data"=>[]]));
		}else{
			$addresses = $this->model_account_address->getAddresses();
			$this->response->setOutput(json_encode(["success"=>1, "error"=>[], "data"=>array_values($addresses)]));
		}
	}
}