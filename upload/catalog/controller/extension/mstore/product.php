...
class ControllerExtensionMstoreProduct extends Controller {
	 ...
	 * @apiParam {Number} [min_price] Filter products by min price
	...
	public function index() {
		...
		$filter_data["filter_price"] = isset($max_price) || isset($min_price);
		...
		foreach ($results as $result) {
			...
			$priceWithTax = $this->tax->calculate($result['price'], $result['tax_class_id'], $this->config->get('config_tax'));
			$specialWithTax = isset($result['special']) ? $this->tax->calculate($result['special'], $result['tax_class_id'], $this->config->get('config_tax')) : 0;

			$check = !isset($max_price) && !isset($min_price);
			if(isset($max_price)){
				$check = ($specialWithTax > 0 && $specialWithTax <= $max_price) || ($specialWithTax == 0 && $priceWithTax <= $max_price);
			}
			if($check && isset($min_price)){
				$check = ($specialWithTax > 0 && $specialWithTax >= $min_price) || ($specialWithTax == 0 && $priceWithTax >= $min_price);
			}
			
			if($check){
				$result['price'] = $priceWithTax;
				$result['price'] = number_format($result['price'], 2, '.', '');

				if($specialWithTax > 0){
					$result['special'] = $specialWithTax;
					$result['special'] = number_format($result['special'], 2, '.', '');
				}
	...
	/**
	 * @api {get} /index.php?route=extension/mstore/product Get product Detail
	 * @apiVersion 0.1.0
	 * @apiName Get products
	 * @apiGroup Catalog
	 *
	...
	public function detail() {
		...
		$result = $this->model_extension_mstore_product->getProduct($productId, $lang);
		if ($result) {
			$options=$this->model_catalog_product->getProductOptions($result["product_id"]);
			$result["options"] = $options;
			$images = $this->model_catalog_product->getProductImages($result["product_id"]);
			...
			$this->response->setOutput(json_encode(["success"=>1, "error"=>[], "data"=>$result]));
		}else{
			$this->response->addHeader('HTTP/1.0 404 Not Found');
			$this->response->setOutput(json_encode(["success"=>0, "error"=>["Product Not Found"], "data"=>null]));
		}
	}
}
