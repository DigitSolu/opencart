...
//		var_dump($target_file, $main_file); die;
...
//		var_dump($target_file); die;
	...
	*/
	protected function getModuleCode($file) {
		...
		$code = preg_replace("/.*\/extension\/([^\/]*)\/([^\/]*)\/.*/is", "$1/$2", $file);
	...
	*/
	protected function getModulesOrder($code, $file, $order = '') {
		...
		//
		$manifest = DIR_APPLICATION . 'controller/extension/' . $code . '/' . KamodManager::MANIFEST_INI_FILE;
