...
abstract class ControllerInstaller extends Controller {
	...
	// contstants
	//
	public static $ka_extensions_version = '4.1.1.9';
	...
	public function install() {
		...
		if (!empty($routes)) {
			foreach($routes as $r) {
				$this->model_user_user_group->addPermission($this->user->getGroupId(), 'access', 'extension/ka_extensions/' . $this->ext_code . '/' . $r);
				$this->model_user_user_group->addPermission($this->user->getGroupId(), 'modify', 'extension/ka_extensions/' . $this->ext_code . '/' . $r);
			}
		}
