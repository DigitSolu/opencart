<?php
class ControllerExtensionCiformbuilderAbout extends Controller {
	private $error = [];
	private $extension_id = 30495;
	private $domain;
	...
	public function __construct($registry) {
		...
		$this->load->language('extension/ciformbuilder/about');
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_setting_setting->editSetting('module_ciformbuilder_key', $this->request->post);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/ciformbuilder/about', $this->module_token .'=' . $this->ci_token . $url, true)
		);
		...
		$data['action'] = $this->url->link('extension/ciformbuilder/about', $this->module_token .'=' . $this->ci_token . $url, true); $data['date'] = $this->getDate('type');

		$data['cancel'] = $this->url->link('common/dashboard', $this->module_token .'=' . $this->ci_token . $url, true);

		if (isset($this->request->post['module_ciformbuilder_key'])) {
			$data['module_ciformbuilder_key'] = $this->request->post['module_ciformbuilder_key'];
		} else {
			$data['module_ciformbuilder_key'] = $this->config->get('module_ciformbuilder_key');
		}
		...
		if(VERSION <= '2.3.0.2') {
			$this->response->setOutput($this->load->view('extension/ciformbuilder/about.tpl', $data));
		} else {
			$file_variable = 'template_engine';
			$file_type = 'template';
			$this->config->set($file_variable, $file_type);
			$this->response->setOutput($this->load->view('extension/ciformbuilder/about', $data));
		}
	}

	public function validateForm() {
		if (!$this->user->hasPermission('modify', 'extension/ciformbuilder/about')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}

		if(empty($this->request->post['module_ciformbuilder_key'])) {
			$this->error['warning'] = 'License key required!';
		}

		if(!empty($this->request->post['module_ciformbuilder_key'])) {
			$key = $this->request->post['module_ciformbuilder_key'];
		} else {
			$key = '';
		}
		...
		if(!$this->error) {
			...
			if(!$this->error && !empty($json_result['success'])) {
				$header = $this->model_extension_ciformbuilder_setting->getHeader($json_result['success'], [$json_result['success'] => 1]);

				$post_data = [];
				foreach($post_info as $key => $value) {
					if($key != 'key' && $key != 'test' && $key != 'server' && $key != 'version') {
						$post_data['ciformbuilder_type_'. mb_substr($key, 0, 1)] = $value;
					}
				}

				$this->model_extension_ciformbuilder_setting->getFooter('ciformbuilder_type', $post_data);
			}
		}
	...
	public function getDate($format = '') {
		...
		$post_info = ['key' => $this->config->get('module_ciformbuilder_key'), 'extension_id' => $this->extension_id, 'domain' => $this->domain, 'server' => $server, 'version' => VERSION, 'test' => $test ];
